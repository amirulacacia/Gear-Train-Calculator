...
namespace MyApp
{
    ...
    void RenderUI()
    {
        ...
        // Engine variables
        static float maxenginespeedrpm, maxenginespeedrad, drivelineefficiency = 100.f;
        static double PIvalue = 3.141592653589793238f;
        static bool torqueoption[3] = { true, false };

        // Torque Equation variables
        static double coefatorque, coefbtorque, coefctorque = 55.24f;
        static double coefa = -4.45f, coefb = 3.17f;
        static int coefapower = -6, coefbpower = -2;

        // Torque coordinate variables
        static double coordx[1000], coordy[1000];
        ...
        if (ImGui::CollapsingHeader("Engine", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (ImGui::BeginTabBar("Torque Option Bar", tab_bar_flags))
            {
                ...
                if (ImGui::BeginTabItem("Data"))
                {
                    ...
                    ImGui::PushItemWidth(50);
                    ImGui::Text("Information of Engine Torque from data coordinates");
                    ImGui::AlignTextToFramePadding();
                    ImGui::Text("Number of points:"); ImGui::SameLine();
                    ImGui::InputInt("(Limits: 100 points)", &numpoints, NULL, NULL);
                    ImGui::PopItemWidth();

                    if (ImGui::Button("Input Coordinate Value"))
                        ImGui::OpenPopup("Coordinate Value");
                    if (ImGui::BeginPopupModal("Coordinate Value", NULL, ImGuiWindowFlags_MenuBar))
                    {
                        ImGui::Text("Input Coordinate Value:");
                        ImGui::Text("x - Engine Speed (rpm)");
                        ImGui::Text("y - Torque (Nm)");
                        for (int i = 0; i < numpoints; i++)
                        {
                            ImGui::PushItemWidth(50);
                            ImGui::AlignTextToFramePadding();
                            ImGui::Text("x%i", i+1); ImGui::SameLine(40); ImGui::Text(":"); ImGui::SameLine(50);
                            ImGui::InputDouble("##enginevalue", &coordx[i], NULL, NULL, "%.1f");
                            ImGui::SameLine(110);
                            ImGui::Text("y%i", i+1); ImGui::SameLine(140); ImGui::Text(":"); ImGui::SameLine(150);
                            ImGui::InputDouble("##torquevalue", &coordy[i], NULL, NULL, "%.1f");
                            ImGui::PopItemWidth();
                        }
        ...
        //static double coordmaxpower, indexpower, coordhighestgear;
        ...
        /*
        indexpower = coordy[0] * coordx[0] * 2.f * PIvalue / 60.f;
        coordmaxpower = 0;
        for (int i = 0; i < numpoints; i++)
        {
            if (coordmaxpower < indexpower)
            {
                coordmaxpower = coordy[i] * coordx[i] * 2.f * PIvalue / 60.f;
                indexpower = coordmaxpower;
            }
        }
        ...
        static double coq, cor, codiscrim, cor13, coterm1, cos, cot, codum1;
        ...
        else if (codiscrim > 0)
        {
            ...
            coroot1 = -coterm1 + cos + cot;
            coterm1 = coterm1 + (cos + cot) / 2.0;
            coroot3im = -coterm1;
            coroot2im = -coterm1;
            coterm1 = sqrt(3.0) * ((-cot + cos) / 2.0);
            coroot2 = coterm1;
            coroot3 = -coterm1;
        }
        else
        ...
        dconstaspeed = coefatorque * 2 * PIvalue / 60;
        dconstbspeed = coefbtorque * 2 * PIvalue / 60;
        dconstcspeed = coefctorque * 2 * PIvalue / 60;
        dconstdspeed = desiredpower * -1;
        ...
        {
            desiredhighestgear = wheelradius * (dgear[0] * 2 * PIvalue / 60) / vehiclespeed;
        }
        else
        {
        ...
        if (ImGui::BeginTabBar("MyTabBar", tab_bar_flags))
        {
            ...
            if (ImGui::BeginTabItem("Geometrical"))
            {
                methodsgear[0] = false;
                methodsgear[1] = true;
                //methodsgear[2] = false;
        ...
        static bool selectedgear[2] = { false, false };
        ...
        if (selectedgear[0])
        {
            ...
            vehiclespeedgraph[gearnumber - 1][1] = speedlimit;
        }
        if (selectedgear[1])
        {
            enginespeedgraph[gearnumber - 1][1] = desiredspeed * ((intgear[gearnumber - 1] / wheelradius) * 60. / 2. / PIvalue * 0.001);
            vehiclespeedgraph[gearnumber - 1][1] = desiredspeed;
        }

        for (int i = (gearnumber - 2), j = 1; i >= 0; i--)
        {
            enginespeedgraph[i][j] = enginespeedgraph[gearnumber - 1][1];
            vehiclespeedgraph[i][j] = (wheelradius / intgear[i]) * (enginespeedgraph[i][j] * 1000. * 2. * PIvalue / 60.);
        ...
        for (int i = 1, j = 0, k = 0; i < (gearnumber + 1); i++, k++)
        {
            enginespeedgraph[i][j] = (intgear[i] / wheelradius) * (vehiclespeedgraph[k][j + 1] * 60. / 2. / PIvalue) * 0.001;
        ...
        for (int i = 0; i < (gearnumber - 1); i++)
        {
            velocitydiff[i] = vehiclespeedgraph[i + 1][1] - vehiclespeedgraph[i][1];
        }
        ...
        static int countvelocitylimit[10];
        ...
        for (int i = 0; i < gearnumber; i++)
        {
            ...
            {
                if (selectedgear[0] == true /* && enginespeed[j] > enginespeedgraph[i][0]  && enginespeed[j] < enginespeedgraph[i][1])
                {
                ...
                else if (selectedgear[1] == true /* && enginespeed[j] > enginespeedgraph[i][0] && enginespeed[j] < enginespeedgraph[i][1])
                {
                    wheelvelocity[i][k] = wheelradius * ((enginespeed[j] * 1000.0 * 2.0 * PIvalue / 60.0) / intgear[i]) * 60.0 * 60.0 / 1000.0;
                    torquewheel[i][k] = intgear[i] * torque[j] * (drivelineefficiency / 100);
                    powerengine[i][k] = power[j] * (drivelineefficiency / 100);
        ...
        if (ImGui::CollapsingHeader("Engine WOT Torque/Power vs Speed Graph", ImGuiTreeNodeFlags_None))
        {
        ...
        if (ImGui::CollapsingHeader("Engine Speed vs Vehicle Speed Graph", ImGuiTreeNodeFlags_None))
        {
            ...
            maxyaxisgear = enginespeedgraph[gearnumber - 1][1] + 1;
            maxxaxisgear = vehiclespeedgraph[gearnumber - 1][1] + 20;
            ImPlot::CreateContext();
            if (ImPlot::BeginPlot("Vehicle Speed vs Engine Speed"))
            {
                ImPlot::SetupAxes("Vehicle Speed (m/s)", "Engine Speed (x1000 rpm)");
                ...
                for (int i = 0; i < gearnumber; i++)
                {
                    ...
                    ImPlot::PlotLine(dotlinename[i], dotlinex[i], dotliney[i], 2, ImPlotCol_Line);
            ...
            for (int i = 0; i < (gearnumber - 1); i++)
            {
                ImGui::Text("Difference V%i - %i: %.1f m/s", i + 1, i + 2, velocitydiff[i]);
            }
        }
        if (ImGui::CollapsingHeader("Torque vs Vehicle Speed Graph", ImGuiTreeNodeFlags_None))
        {
            ImPlot::CreateContext();
            if (ImPlot::BeginPlot("Torque vs Engine Speed"))
            {
                ...
                yaxislimit = maxtorque * intgear[0] * (drivelineefficiency / 100) + 50;
        ...
        if (ImGui::CollapsingHeader("Power vs Engine Speed Graph", ImGuiTreeNodeFlags_None))
        {
            const char* geartag[10] = { "n1##tag", "n2##tag", "n3##tag", "n4##tag", "n5##tag", "n6##tag",
                    "n7##tag", "n8##tag", "n9##tag", "n10##tag" };
            static int yaxislimit = 100;
            static int xaxislimit = 100;
            yaxislimit = maxpower * (drivelineefficiency / 100) / 1000 + 50;
            xaxislimit = speedlimit * 60 * 60 / 1000 + 50;

            ImPlot::CreateContext();
            if (ImPlot::BeginPlot("Power vs Engine Speed"))
            {
